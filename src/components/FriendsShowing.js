    ...
    return (
        <View>
            ...
            <View style={styles.container}>
                {friends.value ? friends.value.map((friend, index) => (
                    <ExTouchableOpacity onPress={() => redirectToProfile(friend.id)} style={styles.item} key={index}>
                        <Image style={styles.itemImage} source={{uri: friend.avatar_image}}/>
                        <Text style={styles.itemName}>{friend.username}</Text>
                    </ExTouchableOpacity>
                )) : ([1,2,3,4,5,6].map((i, index)=> (
                    <View key={index} style={styles.item}>
                        <Image style={styles.itemImage} source={{uri: "https://picsum.photos/536/354"}}/>
